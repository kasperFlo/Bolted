...
function addOrUpdateSite() {
  ...
  const timeLimitInMinutes = parseInt(
  ...
  if (siteName && !isNaN(timeLimitInMinutes)) {
    ...
    siteItem.onclick = () =>
      openEditModal(siteItem, siteName, timeLimitInMinutes);
...
function openEditModal(siteItem, siteName, timeLimitInMinutes) {
  ...
  editingSiteIndex = Array.from(
    document.getElementById("siteList").children
  ).indexOf(siteItem);
}
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  document.querySelectorAll(".addSiteBtn").forEach((button) => {
    button.addEventListener("click", addOrUpdateSite);
  });
